...
var expr = function(incoming_state, incoming_tokens) {
...
var PRECISION = _++;
var COMMENT = _++;
var PREPROCESSOR$1 = _++;
var KEYWORD$1 = _++;
var KEYWORD_OR_IDENT = _++;
var RETURN = _++;
var BREAK = _++;
var CONTINUE = _++;
var DISCARD = _++;
...
function parser() {
  ...
  state.scope = new scope_1(state);
  state.create_node = function() {
    var n = mknode(IDENT$1, token);
  ...
  function write(input) {
    ...
    while(take()) switch(state[0].mode) {
      ...
      case IDENT$1: parse_ident(); break
      case KEYWORD$1: parse_keyword(); break
  ...
  function parse_stmt() {
    ...
    switch(token.type) {
      ...
      default: return state.unshift(expr$$1(';'))
    }
  }
  ...
  function parse_keyword_or_ident() {
    if(token.type === 'keyword') {
      ...
      state[0].mode = KEYWORD$1;
      return
    }

    if(token.type === 'ident') {
      state[0].type = 'ident';
      state[0].mode = IDENT$1;
      return
    }
  ...
  function keyword(default_value) {
    ...
    return mknode(KEYWORD$1, t, node)
  }
  ...
  function assert(type, data) {
    return assert_null_string_or_array(type, token.type) &&
  ...
  function setup_stative_parsers() {
    ...
    parse_struct =
        ...
        , function() {
            if(token.data === '{') {
              state.fake(mknode(IDENT$1, {data:'', position: token.position, type:'ident'}));
              return Advance
            }
    ...
    parse_return =
        ...
        , function() {
            ...
            return state.unshift(expr$$1(';')), Advance
          }
...
var IGNORE = 0xff;
var EXCLUDE_BOTH = 0x00;
var INCLUDE_LEFT = 0x01;
var INCLUDE_RIGHT = 0x02;
var INCLUDE_BOTH = 0x03;
var Range = function Range(left, right, flag) {
    this.left = left;
    this.right = right;
    this.flag = flag;
};

var prototypeAccessors = { isSingle: { configurable: true },includeLeft: { configurable: true },includeRight: { configurable: true },nonFloat: { configurable: true } };
prototypeAccessors.isSingle.get = function () {
    return this.left === this.right;
};
prototypeAccessors.includeLeft.get = function () {
    return this.flag & INCLUDE_LEFT;
};
prototypeAccessors.includeRight.get = function () {
    return this.flag & INCLUDE_RIGHT;
};
prototypeAccessors.nonFloat.get = function () {
    return this.flag === IGNORE;
};
...
Range.create = function create (type, rangeAsStr) {
    if (type === "boolean") 
        { return new Range(0, 1, INCLUDE_BOTH); }
    if (type !== "float" && type !== "integer" && type !== "int") 
        { return Range.none; }
    if (typeof rangeAsStr === "number") 
        { return new Range(rangeAsStr, rangeAsStr, INCLUDE_BOTH); }
    if (!rangeAsStr) 
        { return new Range(Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY, INCLUDE_BOTH); }
    var ref = rangeAsStr.split(",");
        var left = ref[0];
        var right = ref[1];
    var flag = EXCLUDE_BOTH;
    if (left[0] === "[") {
        flag |= INCLUDE_LEFT;
...
function op(opCode, outTypeFn, devectorize) {
    var getOutType = typeof outTypeFn === "function" ? outTypeFn : function () { return outTypeFn; };
    return {
        opCode: opCode,
        getOutType: getOutType,
        devectorize: devectorize
    };
}

function firstArg(inTypes) {
    return inTypes[0];
}

function maxType(inTypes) {
    if (inTypes[0] === "float" && inTypes[1] !== "float") 
        { return inTypes[1]; }
    return inTypes[0];
}

function secondArg(inTypes) {
    return inTypes[1];
}

function thirdArg(inTypes) {
    return inTypes[2];
}

var OpsMap = {
    "+": op(OpCode$1.plus, maxType, devecMath),
    "-": op(OpCode$1.minus, maxType, devecMath),
    "*": op(OpCode$1.mul, maxType, devecMath),
    "/": op(OpCode$1.div, maxType, devecMath),
    "sin": op(OpCode$1.sin, firstArg, devecMath),
    "cos": op(OpCode$1.cos, firstArg, devecMath),
    "atan": op(OpCode$1.atan, firstArg, devecMath),
    "acos": op(OpCode$1.acos, firstArg, devecMath),
    "asin": op(OpCode$1.asin, firstArg, devecMath),
    "min": op(OpCode$1.min, firstArg, devecMath),
    "max": op(OpCode$1.max, firstArg, devecMath),
    "fract": op(OpCode$1.fract, firstArg, devecMath),
    "floor": op(OpCode$1.floor, firstArg, devecMath),
    "power": op(OpCode$1.power, firstArg, devecMath),
    "pow": op(OpCode$1.power, firstArg, devecMath),
    "mix": op(OpCode$1.mix, firstArg, devecMath),
    "normalize": op(OpCode$1.normalize, firstArg, devecMath),
    "dot": op(OpCode$1.dot, "float", devecCompare),
    "sqrt": op(OpCode$1.sqrt, firstArg, devecMath),
    "abs": op(OpCode$1.abs, firstArg, devecMath),
    "mod": op(OpCode$1.mod, firstArg, devecMath),
    "smoothstep": op(OpCode$1.smoothstep, thirdArg, devecMath),
    "unary-": op(OpCode$1.unary_minus, firstArg, devecMath),
    "<": op(OpCode$1.lt, "boolean", devecCompare),
    ">": op(OpCode$1.gt, "boolean", devecCompare),
    "<=": op(OpCode$1.lte, "boolean", devecCompare),
    ">=": op(OpCode$1.gte, "boolean", devecCompare),
    "==": op(OpCode$1.eq, "boolean", devecCompare),
    "||": op(OpCode$1.or, "boolean", devecCompare),
    "&&": op(OpCode$1.and, "boolean", devecCompare),
    "texture2D": op(OpCode$1.texture2D, "vec4", devecCommon),
    "texture": op(OpCode$1.texture2D, "vec4", devecCommon),
    "length": op(OpCode$1.length, "float", devecCommon),
    "step": op(OpCode$1.step, secondArg, devecMath),
    "smoothstep": op(OpCode$1.smoothstep, secondArg, devecMath),
    "clamp": op(OpCode$1.clamp, firstArg, devecMath),
    "sqrt": op(OpCode$1.sqrt, firstArg, devecMath),
    "cross": op(OpCode$1.cross, firstArg, devecMath),
    "vec4": op(undefined, "vec4", devecVecCtor),
    "vec3": op(undefined, "vec3", devecVecCtor),
    "vec2": op(undefined, "vec2", devecVecCtor),
    "=": op(OpCode$1.assign, secondArg, devecMath),
    "_define": op(OpCode$1._define),
    "_ifeq": op(OpCode$1._ifeq, undefined, devecCommon),
    "_endif": op(OpCode$1._endif, undefined, devecCommon),
    "_forget": op(OpCode$1._forget),
    "_output": op(OpCode$1._output, undefined, devecCommon),
    "_watch": op(OpCode$1._watch, undefined, devecNothing),
    "_endwatch": op(OpCode$1._endwatch, undefined, devecNothing),
    "_ignore": op(OpCode$1._ignore, undefined, devecNothing),
    "float": op(OpCode$1._copy, "float", devecNothing),
    "int": op(OpCode$1._copy, "float", devecNothing)
};
function predictOutBuiltinType(operator, inTypes) {
    var knownOp = OpsMap[operator];
    if (knownOp) 
        { return knownOp.getOutType(inTypes); }
    return undefined;
}

function isCopyFn(operator) {
    return OpsMap[operator] && OpsMap[operator].opCode === OpCode$1._copy;
}

var VariablesMap = function VariablesMap() {
    this.nextVarId = 1;
    this.vars = [null];
...
VariablesMap.prototype.addVariable = function addVariable (name, type, initialRange) {
    var variable = {
        name: name,
        type: type,
        range: initialRange,
...
VariablesMap.prototype.getConst = function getConst (value, type) {
    value = value + "";
    return this.constToVar.get(value);
};
VariablesMap.prototype.copyRange = function copyRange (toId, fromId) {
    this.getById(toId).range = this.getById(fromId).range;
};
VariablesMap.prototype.addConst = function addConst (value, type) {
    var id = this.nextVarId++;
    value = value + "";
    var variable = {
        name: ("const(" + value + ")"),
        id: id,
...
VariablesMap.prototype.isConst = function isConst (id) {
    var variable = this.getById(id);
    return variable.value !== undefined;
};
VariablesMap.prototype.getVariables = function getVariables () {
    return this.vars.slice(1);
};
VariablesMap.prototype.isTemp = function isTemp (id) {
    return this.getById(id).isTemp;
};
VariablesMap.prototype.addTempVariable = function addTempVariable (type) {
    var id = this.nextVarId++;
    var variable = {
        name: ("tmp" + id),
        id: id,
...
VariablesMap.prototype.rename = function rename (variable, newName) {
    this.nameToVar.delete(variable.name);
    variable.name = newName;
    this.nameToVar.set(newName, variable);
};
VariablesMap.prototype.getVariable = function getVariable (name) {
    return this.nameToVar.get(name);
};
VariablesMap.prototype.getById = function getById (id) {
    if (typeof id === "object" && id.id !== undefined) {
        id = id.id;
    }
    return this.vars[id];
};
VariablesMap.prototype.enterBlock = function enterBlock () {
    this.block++;
};
VariablesMap.prototype.exitBlock = function exitBlock () {
    this.block--;
};
function noVarPtr() {
    return varPtr(0);
...
function convert(ast, map) {
    if ( map === void 0 ) map = new VariablesMap();

    var out = [];
    var customFunctions = new Map();
    var customFunctionsByName = new Map();
    var functionCallsCounter = 9600;
    var functionCallStack = [];
    var loopCallStack = [];
    function insideLoop() {
    ...
    function predictOutType(operator, inTypes) {
        if (isCustomFunction(operator)) {
            var fullFnId = functionId(operator, inTypes);
            var fn = customFunctions.get(fullFnId);
            if (fn) 
                { return fn.outType; }
    ...
    function getActualVariableName(name, isDeclaration) {
        if (!insideFunction()) 
            { return name; }
        if (isDeclaration) {
            var currentCall = functionCallStack[functionCallStack.length - 1];
            var newName = currentCall.namePrefix + "_" + name;
            currentCall.renamedVars.set(name, newName);
            return newName;
        }
        for (var i = 0, list = functionCallStack.slice().reverse(); i < list.length; i += 1) {
            var call = list[i];
    ...
    function callCustom(name, outVar, argVars) {
        var fid = functionId(name, argVars.map(function (vptr) { return map.getById(vptr).type; }));
        var callId = functionCallsCounter++;
        ...
        var customFunction = customFunctions.get(fid);
        out.push({
            op: "_watch",
            out: [varPtr(outVar)],
            args: [],
            line: customFunction.node.token.line
        });
        var i = 0;
        for (var i$1 = 0, list = customFunction.inArgNames; i$1 < list.length; i$1 += 1) {
            var fnArgName = list[i$1];

            var newName = getActualVariableName(fnArgName, true);
            var variable = map.addVariable(newName, customFunction.inTypes[i], undefined);
            out.push({
                op: "=",
                out: [varPtr(variable)],
                args: [varPtr(argVars[i])],
                line: customFunction.node.token.line
            });
            i++;
        }
    ...
    function call(node) {
        var functionName = node.children[0].token.data;
        var args = node.children.slice(1);
        var argVars = args.map(process);
        var outType = predictOutType(functionName, argVars.map(function (vptr) { return map.getById(vptr).type; }));
        ...
        var variable = map.addTempVariable(outType);
    ...
    function dot(node) {
        var argLeftPtr = process(node.children[0]);
        var part = node.children[1].token.data;
        var varType = ["float","vec2","vec3","vec4"][part.length - 1];
        var variable = map.addTempVariable(varType);
        out.push({
            op: "=",
            out: [varPtr(variable)],
            args: [varPtr(argLeftPtr, part)],
            line: node.token.line
        });
        return varPtr(variable);
    }
    ...
    function arrayItem(node) {
        var name = node.children[0].token.data;
        var idx = node.children[1].token.data;
        var fullName = name + "[" + idx + "]";
        fullName = getActualVariableName(fullName);
        var variable = map.getVariable(fullName);
        if (!variable) 
            { throw new Error("Arrays not supported - array items shall be pre-defined"); }
        return varPtr(variable);
    }
    
    function operator(node) {
        var operator = node.token.data;
        if (operator === '[') {
        ...
        var argVars = node.children.map(process);
        var variable = map.addTempVariable(predictOutType(operator, argVars.map(function (vptr) { return map.getById(vptr).type; })));
        out.push({
            op: operator,
            out: [varPtr(variable)],
            args: argVars,
    ...
    function declvar(node) {
        var c = node.children;
        var decllist = c[c.length - 1];
        var name = getActualVariableName(decllist.children[0].token.data, true);
        var type = c[c.length - 2].token.data;
        var possibleRange = undefined;
        if (decllist.token.preceding) {
            var cmt = decllist.token.preceding.find(function (t) { return t.type === "block-comment"; });
            if (cmt) {
        ...
        var variable = map.addVariable(name, type, possibleRange);
        if (decllist.token.data === ";") {} else {
            var expr = decllist.children[1];
            var inId = process(expr);
    ...
    function declfn(node) {
        var outType = node.children[node.children.length - 2].token.data;
        var fn = node.children[node.children.length - 1];
        var fnName = fn.children[0].token.data;
        var stmtList = fn.children[fn.children.length - 1];
        if (fnName === 'main') {
            process(stmtList);
        } else if (fnName === 'mainImage') {
            process(stmtList);
        } else {
            var declArgs = fn.children[1].children;
            var inTypes = declArgs.map(function (decl) { return decl.children.filter(function (c) { return c.type === "keyword"; }).pop().token.data; });
            var customFn = {
                node: node,
                outType: outType,
                inTypes: inTypes,
                name: fnName,
                inArgNames: declArgs.map(function (decl) { return decl.children[decl.children.length - 1].children[0].token.data; }),
                body: stmtList
            };
    ...
    function decl(node) {
        if (node.children.some(function (c) { return c.type === "decllist"; })) {
            return declvar(node);
        }
        if (node.children.some(function (c) { return c.type === "function"; })) {
            return declfn(node);
        }
    }
    ...
    function assign(node) {
        switch (node.token.data) {
            ...
            case "*=":
            case "/=":
                var modifyOp = node.token.data[0];
                var fakeNode = Object.assign({}, node,
                    {token: Object.assign({}, node.token,
                        {data: modifyOp})});
                var tmpVarPtr = operator(fakeNode);
    ...
    function assignDirect(node) {
        ...
        var argId = process(right);
        var variable = assignDirectToVariable(left, argId);
        if (!variable) {
    ...
    function ifcondition(node) {
        ...
        var thenblock = ref[1];
        var elseblock = ref[2];
        var condVarId = process(cond);
        out.push({
            op: "_ifeq",
            args: [condVarId],
            out: [],
            range: Range.create("float", 1),
            line: thenblock.line
        });
        process(thenblock);
        out.push({
    ...
    function returnexpr(node) {
        if (!insideFunction()) 
            { return console.warn("Return outside function? huh"); }
        var currentCall = functionCallStack[functionCallStack.length - 1];
        var varId = process(node.children[0]);
        out.push({
            op: "=",
            out: [varPtr(currentCall.returnVar)],
            args: [varId],
            line: node.token.line
        });
        out.push({
    ...
    function breakexpr(node) {
        if (!insideLoop()) 
            { return console.warn("Break outside loop? heh"); }
        var currentLoop = loopCallStack[loopCallStack.length - 1];
        out.push({
            op: "_ignore",
            out: [varPtr(currentLoop.iteratorVar)],
            args: [],
            line: node.token.line
        });
    }
    ...
    function getConst(node) {
        switch (node.type) {
            case "literal":
                return parseFloat(node.token.data);
            case "unary":
                if (node.token.data === "-" && node.children[0]) {
                    var out = getConst(node.children[0]);
                    if (out !== undefined) 
                        { return -out; }
                }
        }
    ...
    function simpleloop(node) {
        var initexpr = node.children[0];
        var condexpr = node.children[1];
        var changeexpr = node.children[2];
        var body = node.children[3];
        if (initexpr.type !== "decl") 
            { return false; }
        var iteratorVar = map.getById(declvar(initexpr));
        var initRange = Range.create(iteratorVar.type, iteratorVar.range);
        if (!initRange.isSingle) 
            { return false; }
        var initValue = initRange.left;
        if (condexpr.type !== "expr" && condexpr.children[0].type !== "binary") 
            { return false; }
        condexpr = condexpr.children[0];
        if (condexpr.token.data !== "<" && condexpr.token.data !== ">") 
            { return false; }
        if (condexpr.children[0].type !== "ident") 
            { return false; }
        var limit = getConst(condexpr.children[1]);
        if (limit === undefined) 
            { return false; }
        var step;
        if (changeexpr.type !== "expr") 
            { return false; }
        ...
        out.push({
            op: "_watch",
            out: [varPtr(iteratorVar)],
            args: [],
            line: node.token.line
        });
        var condition = limit > initValue ? function (i) { return i < limit; } : function (i) { return i > limit; };
        for (var i = initValue;condition(i); i += step) {
            var constValue = map.getConst(i) || map.addConst(i, iteratorVar.type);
            out.push({
                op: "=",
                out: [varPtr(iteratorVar)],
                args: [varPtr(constValue)],
                line: node.token.line
            });
            process(body);
        }
    ...
    function process(node) {
        ...
        switch (node.type) {
            ...
            case "keyword":
                if (node.token.data === 'false') 
                    { return createConst("0"); }
                if (node.token.data === 'true') 
...
function getPtrComponents(ptr, map) {
    var variable = map.getById(ptr);
    var allComponents = variable._devectorized;
    if (!allComponents) {
        if (ptr.part) 
            { throw new Error("part for non-vector?"); }
        return [ptr];
    }
    return ptr.part ? ptr.part.split('').map(function (c) { return Components[c]; }).map(function (idx) { return allComponents[idx]; }) : allComponents.slice();
}
...
function devecVecCtor(op, map) {
    ...
    var outArgs = getPtrComponents(op.out[0], map);
    var inArgs = [];
    for (var i = 0, list = op.args; i < list.length; i += 1) {
        var arg = list[i];

        inArgs.push.apply(inArgs, getPtrComponents(arg, map));
    }
...
function devecMath(op, map) {
    var outArgs = getPtrComponents(op.out[0], map);
    var arg1 = getPtrComponents(op.args[0], map);
    var arg1isVector = arg1.length > 1;
    var arg2 = [];
    if (op.args.length > 1) {
    ...
    while (outArgs.length < 4) 
        { outArgs.push(noVarPtr()); }
    if (arg2.length > 0) {
        var vectorPlusSingle = arg2.length === 1 && arg1isVector;
        var singlePlusVector = arg2.length > 1 && !arg1isVector;
        while (arg1.length < 4) 
            { arg1.push(singlePlusVector ? arg1[0] : noVarPtr()); }
        while (arg2.length < 4) 
            { arg2.push(vectorPlusSingle ? arg2[0] : noVarPtr()); }
...
function devecCommon(op, map) {
    var outArgs = op.out.length ? getPtrComponents(op.out[0], map) : [];
    var inArgs = [];
    for (var i = 0, list = op.args; i < list.length; i += 1) {
        var arg = list[i];
...
function ensureNoVectors(op, map) {
    for (var i = 0, list = op.args.concat(op.out); i < list.length; i += 1) {
        var vptr = list[i];

        var variable = map.getById(vptr);
...
function devectorize(ops, map) {
    var out = [];
    for (var i$1 = 0, list = map.getVariables(); i$1 < list.length; i$1 += 1) {
        var variable = list[i$1];

        if (variable.type === undefined) {
            console.warn("Variable ", variable.name, " has no type", variable);
        } else if (variable.type.startsWith("vec")) {
            var vecSize = parseInt(variable.type.substring(3));
            var components = [variable];
            for (var i = 1;i < vecSize; i++) {
                var newVariable = map.addVariable(variable.name + "." + Components.main[i], "float", variable.range);
                components.push(newVariable);
                newVariable.isTemp = variable.isTemp;
            }
            map.rename(variable, variable.name + "." + Components.main[0]);
            variable.type = "float";
            variable._devectorized = components.map(function (v) { return varPtr(v); });
        }
    }
    for (var i$2 = 0, list$1 = ops; i$2 < list$1.length; i$2 += 1) {
        var op = list$1[i$2];

        if (OpsMap[op.op]) {
            var devectorize = OpsMap[op.op].devectorize;
...
function addVarsInOut(ops, map) {
    var out = [];
    var lastSeenPerId = new Map();
    for (var i$2 = 0, list$3 = ops; i$2 < list$3.length; i$2 += 1) {
        var op = list$3[i$2];

        var vars = op.out.concat(op.args);
        var varsToMark = [];
        for (var i = 0, list$1 = vars; i < list$1.length; i += 1) {
            var varptr = list$1[i];

            if (varptr.id === noVarPtr().id) 
                { continue; }
            var variable = map.getById(varptr);
            if (!lastSeenPerId.has(variable.id) && varsToMark.indexOf(variable.id) === -1) {
                out.push({
                    op: "_define",
                    out: [varPtr(variable)],
                    args: [],
                    range: Range.create(variable.type, variable.range)
                });
            }
            varsToMark.push(variable.id);
        }
        out.push(op);
        for (var i$1 = 0, list$2 = varsToMark; i$1 < list$2.length; i$1 += 1) {
            var id = list$2[i$1];

            lastSeenPerId.set(id, out.length);
        }
    }
    var list = Array.from(lastSeenPerId, function (ref) {
        var id = ref[0];
        var line = ref[1];
    ...
    for (var i$3 = 0, list$4 = list; i$3 < list$4.length; i$3 += 1) {
        var ref = list$4[i$3];
        var id$1 = ref.id;
        var line = ref.line;
...
function prepareForAnalysis(ops, map) {
    var out = [];
    var cmdId = 1;
    for (var i = 0, list = ops; i < list.length; i += 1) {
        var op = list[i];

        var knownOp = OpsMap[op.op];
        if (!knownOp) 
            { throw new Error("unknown op: " + op.op); }
        var newOp = {
            cmdId: cmdId,
            opCode: knownOp.opCode,
            args: op.out.concat(op.args).map(function (v) { return v.id; }),
            line: op.line || -1,
            range: op.range || Range.none,
            op: op.op
        };
...
var WarnHighlight = function WarnHighlight(quill$$1) {
    this.el = document.createElement("div");
    this.spanText = document.createElement("div");
    quill$$1.root.parentNode.appendChild(this.el);
    this.el.appendChild(this.spanText);
};
WarnHighlight.prototype.show = function show (rect, text) {
...
WarnHighlight.prototype.hide = function hide () {
    this.el.style.display = "none";
};
var UIController = {
    init: function init(settings, parseSrcIntoReport) {
        var this$1 = this;
        ...
        this.warnHighlights = [];
        this.quill.on('text-change', function () { return this$1.hideWarnings(); });
        ...
        this.tempCheckbox.addEventListener("change", function () {
            this$1.settings.save({
                showTemp: this$1.tempCheckbox.checked
            });
            this$1.showTemp = this$1.tempCheckbox.checked;
            this$1.process();
        });
        this.debugCheckbox = document.querySelector("#debug");
        this.debugCheckbox.checked = this.settings.needDebug;
        this.debugCheckbox.addEventListener("change", function () {
            this$1.settings.save({
                needDebug: this$1.debugCheckbox.checked
            });
            this$1.process();
        });
    ...
    enableButton: function enableButton() {
        ...
        this.checkButton.addEventListener("click", function () { return this$1.process(); });
    },
    hideWarnings: function hideWarnings() {
        var this$1 = this;

        for (var i = 0, list = this$1.warnHighlights; i < list.length; i += 1) {
            var w = list[i];

            w.hide();
        }
    },
    showWarnings: function showWarnings(warnings) {
        var this$1 = this;

        this.hideWarnings();
        var shownForLines = new Set();
        for (var i = 0;i < warnings.length; i++) {
            var w = warnings[i];
            if (shownForLines.has(w.line)) 
                { continue; }
            var wh = this$1.warnHighlights[i];
            if (!wh) {
                wh = new WarnHighlight(this$1.quill);
                this$1.warnHighlights[i] = wh;
            }
            var lines = this$1.quill.getText().split("\n");
            var start = 0, li = (void 0);
            for (li = 0; li < w.line; li++) {
                if (li === w.line - 1) 
                    { break; }
                start += lines[li].length + 1;
            }
            var length = lines[li].length;
            wh.show(this$1.quill.getBounds(start, length), w.text);
            shownForLines.add(w.line);
        }
    },
    showLog: function showLog(report) {
        var this$1 = this;

        var out = [];
        var alreadyPrintedLines = new Set();
        for (var i = 0, list = report.flow; i < list.length; i += 1) {
            var item = list[i];

            if (item.isTemp && !this$1.showTemp) 
                { continue; }
            ...
            if (item.branch) 
                { alreadyPrintedLines.clear(); }
            var line = ("".padStart(item.indent) + item.text).padEnd(60);
            if (item.linecomment) 
                { line += "//" + item.linecomment; }
            out.push(line);
        }
        this.result.innerHTML += '<hr>' + out.join("\n");
    },
    addExampleButton: function addExampleButton(ref) {
        var this$1 = this;
        var name = ref.name;
        var src = ref.src;
        ...
        button.addEventListener("click", function () {
            this$1.quill.setText(src);
            this$1.quill.formatText(0, this$1.quill.getLength(), "code-block", true);
            this$1.process();
        });
    },
    process: function process() {
        ...
        try {
            this.settings.save({
                src: this.quill.getText()
            });
            var report = this.parseSrcIntoReport(this.quill.getText(), this.settings.needDebug);
            var endTime = new Date().getTime();
            if (report.warnings.length) {
                for (var i = 0, list = report.warnings; i < list.length; i += 1) {
                    var w = list[i];
                    ...
                    this$1.result.innerHTML += "<p class='warning'><c>L" + (w.line) + ": </c>" + condition + (w.text) + "</p>";
                }
            } else {
...
var HumanReport = function HumanReport(report, map, ops, src) {
    var this$1 = this;

    function getRealVarName(varId) {
        var variable = map.getById(varId);
        if (!variable) 
            { return {
            print: "?unknown?",
            highlight: null
        ...
        if (name.indexOf(".") !== -1) {
            ...
            var main = ref[0];
            var suffix = ref[1];
            var foundMain = Components.main.split('').some(function (ms) { return src.indexOf((main + "." + ms)) != -1; });
            var foundColor = Components.color.split('').some(function (ms) { return src.indexOf((main + "." + ms)) != -1; });
    ...
    function formatVarChange(varChange) {
        var varName = getRealVarName(varChange.varId);
        var newRange = varChange.range;
        return ((varName.print) + " ∈ " + (newRange.toString()));
    }
        
    function getOriginalOp(cmdId) {
        return ops.find(function (op) { return op.cmdId == cmdId; });
    }
        
    var branches = report.filter(function (b) { return b.type === "branch"; });
    ...
    var loop = function () {
        ...
        b.children = [];
        var varChange = report.find(function (c) { return c.branchId == b.branchId && c.type == "change" && c.cmdId == b.cmdId && c.varId == b.varId; });
    ...
    for (var i = 0, list = branches; i < list.length; i += 1) loop();
    for (var i$1 = 0, list$1 = branches; i$1 < list$1.length; i$1 += 1) {
        var b$1 = list$1[i$1];

        if (b$1.branchId != 0) {
            b$1.parent = branchesMap.get(+b$1.parentId);
            b$1.parent.children.push(b$1);
            b$1.parent.endCmdId = b$1.cmdId;
        }
    }
    this.branches = branches;
    function getFullBranchConditions(branchId) {
        var branch = branchesMap.get(+branchId);
        var conds = [];
        while (branch != null) {
            if (branch.reason) 
                { conds.unshift(branch.reason); }
    ...
    function findLastChangeForVar(varId, branchId) {
        var b = branchesMap.get(branchId);
        while (b) {
            var lastChange = report.filter(function (r) { return r.type == "change" && r.varId == varId && r.branchId == b.branchId; }).pop();
            if (lastChange) 
                { return lastChange; }
            b = b.parent;
        }
    ...
    var dupWarnings = report.filter(function (r) { return r.type === "warning"; }).map(function (w) {
        var op = getOriginalOp(w.cmdId);
        var text = "Call to " + (op.op) + ": " + (getRealVarName(w.varId).print) + " expected to fit " + (w.expectedRange) + ", but it is " + (w.actualRange);
        if (op.opCode === OpCode$1._output) {
            text = (getRealVarName(w.varId).print) + " expected to fit " + (w.expectedRange) + ", but it is " + (w.actualRange);
        }
        var line = op.line;
        if (!line || line == -1) {
            var lastChange = findLastChangeForVar(w.varId, w.branchId);
            if (lastChange) {
    ...
    var loop$1 = function () {
        ...
        if (w.isDuplicate) 
            { return; }
        var duplicates = dupWarnings.filter(function (dw) { return dw.text == w.text && dw != w; });
        var branches$1 = [w.branchId];
        if (duplicates.length) {
            for (var i$2 = 0, list$2 = duplicates; i$2 < list$2.length; i$2 += 1) {
                var dup = list$2[i$2];

                branches$1.push(dup.branchId);
                dup.isDuplicate = true;
            }
        }
        var conditions = branches$1.map(getFullBranchConditions).filter(function (c) { return c.trim().length; });
        var str = conditions.length === 1 ? conditions[0] : conditions.map(function (c) { return "(" + c + ")"; }).join(" || ");
        ...
        this$1.warnings.push(w);
    };

    for (var i$3 = 0, list$3 = dupWarnings; i$3 < list$3.length; i$3 += 1) loop$1();
    var flow = this.flow = [];
    var lines = src.split("\n");
    ...
    function fillBranch(branch, indent) {
        ...
        flow.push(item);
        var loop = function ( cmdId ) {
            for (var i = 0, list = report.filter(function (c) { return c.type == "change" && c.cmdId == cmdId && c.branchId == branch.branchId; }); i < list.length; i += 1) {
                var c = list[i];

                if (getOriginalOp(c.cmdId).opCode == OpCode$1._define) 
                    { continue; }
                item = {
                    indent: indent + 2,
                    varId: c.varId,
        ...
        for (var cmdId = branch.cmdId;cmdId < branch.endCmdId; cmdId++) loop( cmdId );
        for (var i$1 = 0, list$1 = branch.children; i$1 < list$1.length; i$1 += 1) {
            var cb = list$1[i$1];
...
var NullPrinter = function NullPrinter () {};

NullPrinter.prototype.printTree = function printTree (tree) {};
NullPrinter.prototype.printVars = function printVars (vars) {};
NullPrinter.prototype.printAfterPass1 = function printAfterPass1 (out) {};
NullPrinter.prototype.printAfterPass2 = function printAfterPass2 (out) {};
NullPrinter.prototype.printAfterPass3 = function printAfterPass3 (out) {};
NullPrinter.prototype.printAfterPass4 = function printAfterPass4 (out) {};
NullPrinter.prototype.printCommand = function printCommand (cmd) {};
NullPrinter.prototype.printCommandResult = function printCommandResult (cmd) {};
NullPrinter.prototype.printResult = function printResult (response) {};
NullPrinter.prototype.beforeCommandLoop = function beforeCommandLoop () {};
NullPrinter.prototype.afterCommandLoop = function afterCommandLoop () {};
var DebugPrinter = function DebugPrinter(variablesMap) {
    var idToStr = function (varPtr) {
        if (varPtr.id === 0) 
            { return '_'; }
        var variable = variablesMap.getById(varPtr.id);
        if (!variable) 
            { return ("?" + (varPtr.id)); }
...
DebugPrinter.prototype.printTree = function printTree (tree) {
    console.group("tree");
    printNode(tree);
    console.groupEnd();
};
DebugPrinter.prototype.printVars = function printVars (vars) {
    console.log(vars);
};
DebugPrinter.prototype.printAfterPass1 = function printAfterPass1 (out) {
    console.groupCollapsed("pass1");
    console.log(out.map(this.op2string).join("\n"));
    console.groupEnd();
};
DebugPrinter.prototype.printAfterPass2 = function printAfterPass2 (out) {
    console.groupCollapsed("pass2");
    console.log(out.map(this.op2string).join("\n"));
    console.groupEnd();
};
DebugPrinter.prototype.printAfterPass3 = function printAfterPass3 (out) {
    console.groupCollapsed("pass3");
    console.log(out.map(this.op2string).join("\n"));
    console.groupEnd();
};
DebugPrinter.prototype.printAfterPass4 = function printAfterPass4 (out) {
    console.groupCollapsed("pass4");
    var o = [];
    for (var i = 0, list = out; i < list.length; i += 1) {
        var op = list[i];

            o.push([op.cmdId,op.opCode,'(' + op.op + ')',op.args.toString(),op.range.left,
            op.range.right,op.range.flag].join(','));
    }
    console.log(o.join('\n'));
    console.groupEnd();
};
DebugPrinter.prototype.beforeCommandLoop = function beforeCommandLoop () {
    console.groupCollapsed("process");
};
DebugPrinter.prototype.printCommand = function printCommand (op) {
    console.log('process', op.cmdId);
};
DebugPrinter.prototype.afterCommandLoop = function afterCommandLoop () {
    console.groupEnd();
};
DebugPrinter.prototype.printCommandResult = function printCommandResult (ret, errorMessage) {
    console.log(' = ', ret, errorMessage);
};
DebugPrinter.prototype.printResult = function printResult (report) {
...
function printNode(node, indent) {
    ...
    var preceding = "";
    if (node.token) {
        if (node.type !== node.token.type) 
            { type += ":" + node.token.type; }
        if (!node.data) 
            { data = node.token.data; }
        if (data !== node.token.data) 
            { data = node.data + ":" + node.token.data; }
        if (node.token.preceding && node.token.preceding.some(function (t) { return t.type === "block-comment"; })) {
            preceding = "//" + node.token.preceding.find(function (t) { return t.type === "block-comment"; }).data;
        }
    }
    if (!node.type.startsWith("placeholder")) 
        { console.log((("".padStart(indent)) + " " + line + ": " + type + " = " + data + preceding)); }
    for (var i = 0, list = node.children || []; i < list.length; i += 1) 
        {
        var c = list[i];
...
function getOutVarId(map) {
    var outVar = map.getVariable("fragColor");
    if (!outVar) {
...
var DEBUG$1 = true;
function process(src, needDebug) {
    if ( needDebug === void 0 ) needDebug = DEBUG$1;

    var map = new VariablesMap();
    var logger = needDebug ? new DebugPrinter(map) : new NullPrinter();
    var tree = direct(string(src));
    addCommonVars(map);
    if (src.indexOf("mainImage(") !== -1 && src.indexOf("gl_FragColor") === -1) {
        addShadertoyVars(map);
    }
    var pass1 = convert(tree, map);
    var outVarId = getOutVarId(map);
    if (outVarId) {
        pass1.push({
            op: "_output",
            out: [],
    ...
    var pass2 = devectorize(pass1, map);
    logger.printAfterPass2(pass2);
    var pass3 = addVarsInOut(pass2, map);
    logger.printAfterPass3(pass3);
    var pass4 = prepareForAnalysis(pass3, map);
    logger.printAfterPass4(pass4);
    var flow = new DataFlowApi();
    flow.init();
    logger.beforeCommandLoop();
    for (var i = 0, list = pass4; i < list.length; i += 1) {
        var op = list[i];

        logger.printCommand(op);
        var ret = flow.addCommand(op.cmdId, op.opCode, op.args, op.range.left, op.range.right, op.range.flag);
        logger.printCommandResult(op, getErrorMessage(ret));
        if (ret !== 0) 
            { throw new Error("Error during processing command#" + op.cmdId + "(" + op.op + "): " + getErrorMessage(ret)); }
    }
...
function getErrorMessage(ret) {
    return ["OK","Unknown error","Non-existent opCode","Unknown variable","Argument absent"][ret] || "Unknown error";
}

function fixReport(report) {
    for (var i = 0, list = report; i < list.length; i += 1) {
        var item = list[i];
...
window.DataFlowApi.promise.then(function () {
    UIController.enableButton();
    UIController.process();
});
var UISettings = function UISettings () {};

var prototypeAccessors$1 = { showTemp: { configurable: true },needDebug: { configurable: true },src: { configurable: true } };

UISettings.prototype.load = function load () {
...
UISettings.prototype.save = function save (update) {
    this.settings = Object.assign({}, this.settings, update);
    window.localStorage.setItem("settings", JSON.stringify(this.settings));
};
prototypeAccessors$1.showTemp.get = function () {
    return this.settings.showTemp;
};
prototypeAccessors$1.needDebug.get = function () {
    return this.settings.needDebug;
};
prototypeAccessors$1.src.get = function () {
    return this.settings.src;
};

Object.defineProperties( UISettings.prototype, prototypeAccessors$1 );
var settings = new UISettings().load();
UIController.init(settings, function (src, needDebug) { return process(src, needDebug); });
for (var i = 0, list = Examples; i < list.length; i += 1) {
    var ex = list[i];
